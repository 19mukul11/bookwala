...
$row_a = mysqli_fetch_assoc(mysqli_query($conn,"select admin_degination from admin where admin_id = '{$_SESSION['uid']}'"));
if($row_a['admin_degination'] === 'admin'){
    switch ($choice) {
        case 'a':
            admin();
            break;
        ...
        case 'o':
            order();
            break;
        default : $output .= "<h2>can not find any record</h2>";
            break;
    }
}else if($row_a['admin_degination'] === 'manager'){
    switch ($choice) {
        case 'c':
            customer();
            break;
        default:
            $output .= "<h2>you not access this record or This record is not present</h2>";
            break;
    }
}else if($row_a['admin_degination'] === 'accountant'){
    switch ($choice) {
        case 'o':
            order();
            break;
...
}else if($row_a['admin_degination'] === 'store manager' || $row_a['admin_degination'] === 'inventory manage'){
    switch ($choice) {
        case 'b':
            store();
            break;
